...
from Deepak import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, Legend, LegendBot, bot
from Deepak.Config import Config
from Deepak.helpers.exceptions import CancelProcess
from var import Var
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = Deepak.utils
        mod.Config = Config
        ...
        sys.modules["LEGENDBOT.utils"] = Deepak.utils
        sys.modules["LEGENDBOT"] = Deepak
        # support for paperplaneextended
        sys.modules["Deepak.events"] = Deepak.utils
        spec.loader.exec_module(mod)
        # for imports
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Deepak/plugins/assistant/{shortname}.py")
        name = "Deepak.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["Deepak.plugins.assistant" + shortname] = mod
...
def start_spam(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Deepak/plugins/Spam/{shortname}.py")
        name = "Deepak.plugins.Spam.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
...
def load_addons(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Deepak/plugins/Xtra_Plugin/{shortname}.py")
        name = "Deepak.plugins.LegendBot-Addons.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        path = Path(f"Deepak/plugins/Xtra_Plugin/{shortname}.py")
        name = "Deepak.plugins.Xtra_Plugin.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        mod.Legend = Legend
        mod.bot = Legend
...
def load_abuse(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Deepak/plugins/Abuse/{shortname}.py")
        name = "Deepak/plugins/Abuse.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            ftext += "--------BEGIN Deepak TRACEBACK LOG--------"
            ftext += "\nDate: " + date
