...
from VcPlayer import call_py, VcPlayer, client as Client
owner = "1669178360"
from VcPlayer.helpers.yt_dlp import bash
from VcPlayer.helpers.chattitle import CHAT_TITLE
from VcPlayer.helpers.queues import (
    QUEUE,
    add_to_queue,
    clear_queue,
    get_queue,
...
from VcPlayer.helpers.thumbnail import gen_thumb
...
@VcPlayer.on(events.callbackquery.CallbackQuery(data="cls"))
async def _(event):
...
#play
@VcPlayer.on(events.NewMessage(pattern="^[?!/]play"))
...
#end
@VcPlayer.on(events.NewMessage(pattern="^[/?!]end"))
@is_admin
...
@VcPlayer.on(events.NewMessage(pattern="^[?!/]vplay"))
...
#playlist
@VcPlayer.on(events.NewMessage(pattern="^[?!/]playlist"))
@is_admin
...
#leavevc
@VcPlayer.on(events.NewMessage(pattern="^[?!/]leavevc"))
@is_admin
...
@VcPlayer.on(events.NewMessage(pattern="^[?!/]skip"))
@is_admin
...
@VcPlayer.on(events.NewMessage(pattern="^[?!/]pause"))
@is_admin
...
@VcPlayer.on(events.NewMessage(pattern="^[?!/]resume"))
@is_admin
