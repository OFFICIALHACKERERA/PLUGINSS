from VcPlayer import VcPlayer, BOT_USERNAME
...
@VcPlayer.on(events.NewMessage(pattern="^[?!/]start$"))
async def start(event):
    ...
    if event.is_private:
       await event.client.send_file(event.chat_id,
        ...
        [Button.url("👨‍💻 ꜱᴏᴜʀᴄᴇ ᴄᴏᴅᴇ", "https://github.com/ITZ-VcPlayer/Telethon-Music")],
...
@VcPlayer.on(events.callbackquery.CallbackQuery(data="start"))
async def _(event):
    if event.is_private:
