...
@VcPlayer.on(events.NewMessage(pattern="^[!?/]lock ?(.*)"))
@is_admin
async def lock(event, perm):
    ...
    if "text" in input_str:
       ...
       await VcPlayer.edit_permissions(event.chat_id,
          send_messages=False, 
          send_media=False,
          send_stickers=False,
          send_gifs=False,
...
@VcPlayer.on(events.NewMessage(pattern="^[!?/]unlock ?(.*)"))
@is_admin
...
@VcPlayer.on(events.NewMessage(pattern="^[!?/]locktypes"))
...
@VcPlayer.on(events.callbackquery.CallbackQuery(data="locks"))
async def _(event):
