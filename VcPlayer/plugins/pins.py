...
@VcPlayer.on(events.NewMessage(pattern="^[?!/]pinned"))
async def get_pinned(event):
    chat_id = (str(event.chat_id)).replace("-100", "")

    Ok = await VcPlayer.get_messages(event.chat_id, ids=types.InputMessagePinned()) 
...
@VcPlayer.on(events.NewMessage(pattern="^[!?/]pin ?(.*)"))
@is_admin
async def pin(event, perm):
    ...
    if "notify" in input_str:
       await VcPlayer.pin_message(event.chat_id, msg, notify=True)
       return
    await VcPlayer.pin_message(event.chat_id, msg)   

@VcPlayer.on(events.NewMessage(pattern="^[!?/]unpin ?(.*)"))
@is_admin
...
@VcPlayer.on(events.NewMessage(pattern="^[!?/]permapin"))
@is_admin
async def permapin(event, perm):
    ...
    if not msg:
       ...
       return
    hn = await VcPlayer.send_message(event.chat_id, msg.message)
    await VcPlayer.pin_message(event.chat_id, hn, notify=True)


@VcPlayer.on(events.NewMessage(pattern="^[!?/]unpinall"))
...
"""
    ...
    await VcPlayer.send_message(event.chat_id, UNPINALL, buttons=[
    [Button.inline("Confirm", data="unpin")], 
    [Button.inline("Cancel", data="cancel")]])

@VcPlayer.on(events.callbackquery.CallbackQuery(data="unpin"))
async def confirm(event):
    check = await event.client.get_permissions(event.chat_id, event.sender_id)
    if check.is_creator:
        await VcPlayer.unpin_message(event.chat_id)
        await event.edit("Unpinned All Msgs!")
        return 

    await event.answer("Group Creator Required!")

@VcPlayer.on(events.callbackquery.CallbackQuery(data="cancel"))
...
@VcPlayer.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
